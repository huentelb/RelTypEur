...
# load results from LCA if not already available in environment
...
# Set selected model (store for automated selection in following code and labels, etc.)
best <- "lc4"
lc <- lc4
...
## Classification criteria (aBIC, cAIC,...) -> store in word
...
# Average posterior latent class probability -> store in word
avepp <- 
...
# class.lab.p <- as.character(c(paste0("Tight-knit (", post[1,3], "%)"),
#                             paste0("Detached (", post[2,2], "%)"),
#                             paste0("Ambivalent (", post[3,4], "%)"),
#                             paste0("Intimate-but-distant (", post[4,1], "%)")))
