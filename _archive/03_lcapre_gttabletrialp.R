...
kin_name <- kin %>% 
  ...
  filter(kin_nam == 1) 
median(kin_name$rel_cnf, na.rm = TRUE)
median(kin_name$rel_cnt, na.rm = TRUE)
median(kin_name$rel_tra, na.rm = TRUE)
median(kin_name$rel_clo, na.rm = TRUE)
...
lca.df <- kin %>%
  ...
  mutate(cnf_lca = ifelse(rel_cnf <= 1 | is.na(rel_cnf), 1, 2)) %>% # 1. none at all - 4. a great deal (at least "a little" 2)
  mutate(cnt_lca = ifelse(rel_cnt >= 4 | is.na(rel_cnt), 1, 2)) %>% # 1. daily - 6. never (at least "once per month" 3)
  mutate(tra_lca = ifelse(rel_tra >= 5 | is.na(rel_tra), 1, 2)) %>% # 1. same building - 5. >=1 hrs (within 1 hour)
  mutate(clo_lca = ifelse(rel_clo <= 3 | is.na(rel_clo), 1, 2)) %>% # 1. not at all - 5. very close (at least "very close")
...
png(file = paste0(folder.graph, paste0("hist_sup.png")), 
...
png(file = paste0(folder.graph, paste0("per_sup.png")), 
    width = 800, height = 500)
lca.df %>% 
  filter(kin_nam == 1, !is.na(rel_sup),
         kin_nam == 1) %>% 
...
# Reorder the table: First by variable, then by kin category
# final_table <- final_table %>%
#   arrange(Variable, kin_cat.l)

# Create gt table with precise border formatting
gt_tbl <- final_table %>%
  ...
  tab_options(
    ...
    table_body.hlines.style = "none", # removed horizontal lines from cells
    stub.border.style = "none", # removed vertical lines
    stub_row_group.border.style = "none",
    summary_row.border.style = 'none'
    
    ) %>%
  tab_style(
    style = cell_borders(sides = c("top", "bottom"), color = "black", weight = px(.5)),
...
# Save table
gtsave(gt_tbl, paste0(folder.graph, "TabA1.html"))
gtsave(gt_tbl, paste0(folder.graph, "TabA1.pdf"))
gtsave(gt_tbl, paste0(folder.graph, "TabA1.docx"))

gtsave(gt_tbl, filename = file.path(folder.graph, "TabA1.docx"))
...
lca.df$rel_tra.l[USA$rel_tra.l == "-5. Incomplete data"] <- "5. 1 hour or longer" 
lca.df$rel_tra.l[USA$rel_tra.l == "-1. Don't know"] <- "5. 1 hour or longer" 
...
lca.df$rel_cnt[is.na(USA$rel_cnt)] <- 6 # never contact 
lca.df$rel_cnt.l[USA$rel_cnt.l == "-5. Incomplete data"] <- "6. Never" 
...
lca.df$rel_clo[is.na(USA$rel_clo)] <- 1 # not at all close
lca.df$rel_clo.l[USA$rel_clo.l == "-5. Incomplete data"] <- "1. Not at all close" 
...
lca.df$rel_cmf1.l[USA$rel_cmf1.l == "-5. Incomplete data"] <- "1. Did not comfort" 
...
lca.df$rel_cou1.l[USA$rel_cou1.l == "-5. Incomplete data"] <- "1. Could not count on" 
...
lca.df$rel_mon1[is.na(USA$rel_mon1)] <- 1 # not given
lca.df$rel_mon1.l[USA$rel_mon1.l == "-5. Incomplete data"] <- "1. Has not given money" 
...
lca.df$rel_cnf[is.na(USA$rel_cnf)] <- 1 # no conflict at all
lca.df$rel_cnf.l[USA$rel_cnf.l == "-5. Incomplete data"] <- "1. None at all" 
