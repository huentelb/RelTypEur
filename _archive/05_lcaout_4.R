...
ftab <- footnote(ftab, 
                 ...
                 j = c(2, 3, 4, 5),
                 ref_symbols = "a",
                 value = as_paragraph("Note: Values ≥.90 are ideal; values <.80 or (see Weller et al. 2020) or <.70 are unacceptable (see Maysn, 2013)"))
...
class.lab.p <- as.character(c(paste0("Close (", post[1,2], "%)"),
                            paste0("Ambivalent (", post[2,2], "%)"),
                            paste0("Supportive (", post[3,2], "%)"),
                            paste0("Distant (", post[4,2], "%)")))
...
ggplot(lcmodel,aes(x = L2, y = value, fill = Var2)) + 
  ...
  scale_x_discrete(labels = c("clo_lca" = "Closeness",
                              "cnf_lca" = "Conflict",
                              "cnt_lca" = "Contact",
                              "mon_lca" = "Money",
                              "sup_lca" = "Support",
                              "tra_lca" = "Proximity"),
                   limits = c("cnf_lca", "cnt_lca", "clo_lca", "mon_lca", "sup_lca", "tra_lca")) +
  theme(
